...
#
# splines_dict = {
#   'types' : (nsplines,) int array # int in BEZIER, POLY, NURBS
#   'cyclic' : (nsplines,) bool array
#   'resolution' : (nsplines,) int array
#   'material_index' : (nsplines,) int array
#   'splines' : array of spline_dict
#   }
#
# spline_dict: {
#     'type': 'BEZIER' | 'NURB' | 'POLY',
#     'points': (n, 3) | (n, 4) float array,
#     'handle_left': (n, 3) float array,
#     'handle_right': (n, 3) float array
#     'handle_type_left': (n,) int array,
#     'handle_type_right': (n,) int array
#     'radius' (n,) float array
#     'tilt' (n,) float array
#   }
#
# ====================================================================================================
...
def set_spline_points(bl_spline, spline):
    ...
    def write(coll, attr, a, dtype=None):
        if a is None:
            return
        if dtype is None:
            coll.foreach_set(attr, np.reshape(a, np.size(a)))
        else:
            coll.foreach_set(attr, np.reshape(a, np.size(a)).astype(dtype))
    ...
    if bl_spline.type == 'BEZIER':
        write(bl_spline.bezier_points, 'co',                spline['points'],          bfloat)
        write(bl_spline.bezier_points, 'handle_left',       spline.get('handle_left'), bfloat)
        write(bl_spline.bezier_points, 'handle_right',      spline.get('handle_right'),bfloat)
        # Key name is the one of Geometry Nodes which is different from python name
        ...
        write(bl_spline.bezier_points, 'radius', spline.get('radius'), bfloat)
        write(bl_spline.bezier_points, 'tilt',   spline.get('tilt'),   bfloat)

    else:
        write(bl_spline.points, 'co',     spline['points'],     bfloat)

        write(bl_spline.points, 'radius', spline.get('radius'), bfloat)
        write(bl_spline.points, 'tilt',   spline.get('tilt'),   bfloat)
...
def set_splines(spec, splines_dict, update=True):
    ...
    for key, a in splines_dict.items():
        ...
        bl_splines.foreach_set(attr_names[key], np.reshape(a, np.size(a)).astype(vtype))
