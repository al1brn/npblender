...
from npblender import KDTree

from npblender.maths.transformations import Transformations, axis_vector, normalize, tracker, reflect
from npblender.maths.functions import Function, keyed
from npblender.maths import distribs, minmax

from npblender.core.simulation import Action, Simulation
...
class Force(Action):
    def __init__(self, func, *args, start=0, duration=None, is_force=True, **kwargs):
        """ A force or acceleration

        A force increments the acceleration property of points in a <!Kinematics> simulation.
        The acceleration is the force divided by the mass. The force can also be directly an acceleration,
        for instance gravity acceleration.

        Properties
        ----------
        - func (function) : function to call
        - args (*args) : arguments to pass the the function
        - is_force (bool = True) : Force or acceleration
        - kwargs : keyword arguments to pass to the function
        """

        super().__init__(func, *args, start=start, duration=duration, **kwargs)
...
class Particles(Simulation):

    def __init__(self, object=None, count_down=False, max_force=None, max_acc=None, min_speed=None, max_speed=None):
        """ > Particles animation
        ...
        Properties
        ----------
        - points (points) : points domain of <!Simulation#Geometry>
        ...
        self.points.new_vector('speed',        default=(0, 0, 0))
        self.points.new_vector('acceleration', default=(0, 0, 0))
        ...
        self.points.add_auto_attribute('last_pos',     'VECTOR'    , default = (0, 0, 0))
        self.points.add_auto_attribute('viscosity',    'FLOAT'     , default = .01)

        self.points.add_auto_attribute('scales',       'VECTOR'    , default = (1, 1, 1))

        # ----- Rotation

        self.points.add_auto_attribute('moment',       'FLOAT'     , default = 1.)
        self.points.add_auto_attribute('eulers',       'VECTOR'    , default = (0, 0, 0))
        self.points.add_auto_attribute('omega',        'VECTOR'    , default = (0, 0, 0))
    ...
    @property
    def ignore_acc(self):
        """ > Points selection where acceleration must be ignored

        Since the number of points can vary, this selection is created at each exec loop.

        Returns
        -------
        - array of bools
        """
        if self._ignore_acc is None:
            self._ignore_acc = np.zeros(0, len(self.points), bool)
        return self._ignore_acc
    ...
    def compute(self):
        ...
        if self._ignore_acc is not None:
            acc[self._ignore_acc] = 0
            # Reset for next step
            self._ignore_acc = None
        ...
        new_speed = self.points.speed + acc*self.delta_time
        new_speed = minmax.clip_vectors(new_speed, self.min_speed, self.max_speed, delta=.1)

        self.points.position += (self.points.speed + new_speed)*(self.delta_time/2)
    ...
    def add_particles(self, points, **attrs):
        """ > Add particles

        Arguments
        ---------
        - points (array of vectors) : position of the particles to add
        - attrs : points attributes
        """
    ...
    def set_count(self, count):
        ...
        if diff < 0:
            ...
            self.points.add_points(np.zeros((diff, 3), float))
    ...
    def jet_engine(self, location=(0, 0, 0), direction=(0, 0, 1), radius=1, speed=50, density=500):
        """ > Jet engine particles
        """
        dist  = distribs.disk_dist(radius=radius, density=density*self.delta_time, seed=self.rng)
        if dist is None:
            return

        speed = distribs.speed_dist(dist['normals'], speed=speed/(.01 + dist['lengths']), scale=None, mu=None, seed=self.rng)

        q = tracker(direction, '-Z')

        self.add_particles(location + q @ dist['points'], speed=q @ speed, age=0, density=np.linalg.norm(dist['points'], axis=-1))
    ...
    # ====================================================================================================
    # Predefined forces and acceleration

    # ----------------------------------------------------------------------------------------------------
    # Gravity : a constant acceleration

    @staticmethod
    def gravity(kinematics, g=-9.86):
        g = kinematics.keyed(g)
    ...
    @staticmethod
    def newton_law(kinematics, G=1, power=2):
        ...
        if not len(kinematics.points):
            return

        v = kinematics.points.position - kinematics.points.position[:, None]
        ...
        d *= G*kinematics.points.mass

        a = np.sum(d[..., None]*v, axis=1)

        return a

        #kinematics.points.acceleration += a

    # ----------------------------------------------------------------------------------------------------
    # Central acceleration

    @staticmethod
    def central_acc(kinematics, location=(0, 0, 0), factor=1., power=-2, max_acc=None):

        rel_locs = kinematics.points.position - location
        ...
        return (-d2)[..., None]*kinematics.points.position

    # ----------------------------------------------------------------------------------------------------
    # Centrifugal and Coriolis acceleration

    @staticmethod
    def centrifugal(kinematics, location=(0, 0, 0), omega=1, axis='Z', coriolis_factor=1.):
        ...
        acc += np.cross(-2*omega*axis, kinematics.points.speed)*coriolis_factor
    ...
    @staticmethod
    def viscosity(kinematics, factor=1., power=2, max_force=None, fluid_speed=None):

        # ------ Points speed relative to the speed of the fluid

        if fluid_speed is None:
            rel_speed = kinematics.points.speed
        else:
            rel_speed = kinematics.points.speed - fluid_speed
        ...
        force = factor*kinematics.points.viscosity*(speeds**power)

        # ----- The force can't inverse the speed

        max_f = kinematics.points.mass*speeds/kinematics.delta_time
    ...
    def to_volume_grid(self, volume, grid_name, attribute, resolution=1):

        volume.new_grid(grid_name, points=self.points.position, values=self.points.attributes[attribute], resolution=resolution)

        return volume


    def to_volume(self, name, shape=(100, 100, 100), position='density', **grid_names):

        from npblender import Volume

        vol = Volume(name)

        attrs = {'position': position, **grid_names}

        for attr, grid_name in attrs.items():
            vol.new_grid(grid_name, shape=shape, points=self.points.attributes[attr])
