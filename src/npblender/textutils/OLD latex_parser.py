...
class Token:
    ...
    KINDS = ['command', 'token', 'text', 'space', 'special', 'symbol', 'comment']
...
class Parser:
    ...
    def _read_command(self):
        ...
        if c.isalpha():
            ...
            if name in SYMBOLS:
                return Token.token(SYMBOLS[name])
            else:
    ...
    def _read_word(self):
        ...
        while True:
            ...
            if ch is None or not ch.isalpha():
                break
            self._advance(1)
        return Token.token(self.s[start:self.i])
    ...
    def _read_comment(self):
        ...
        while not self._eof():
            if self._peek() == "\n":
                break
            self._advance(1)
        return Token.comment(self.s[start:self.i])
    ...
    def next_math_token(self):
        ...
        while not self._eof():
            ...
            # comment
            if not self.ignore_comments and (ch == "%"):
                comment = self._read_comment()
                if self.return_comments:
                    return comment
    ...
    def _read_text_item(self, mandatory=False):
        ...
        while True:
            ...
            if token.kind == 'special':
                ...
                if token.value == '{':
                    return self.read_block(Token.special('}'), block_type='BLOCK')

                elif token.value == '}':
                    return {'type': 'close', 'value': token.value}
                
                elif token.value == '~':
                    return {'type': 'CONTENT', 'content': ['~']}
                
                elif token.value in ['$', '$$']:
                    self.read_block(token.value, to_mode='MATH', block_type='MATH')
            ...
            # ---------------------------------------------------------------------------
                ...
                if cmd in ['(', '[']:
                    end_token = token.command('\\)' if cmd == '(' else '\\]')
                ...
                if cmd in TEXT_SWITCHES:
                    ...
                    return self.read_block(Token.special('}'), to_mode='TEXT', block_type='TEXT', **TEXT_BLOCKS[cmd])
            ...
            elif token.kind in ['text', 'token']:
                return {'type': 'STRING', 'string': token.value}

            # Shouldn't occur

            raise RuntimeError(f"LaTeX text parser error: : {token}")
        ...
        while True:
            ...
            if token.kind == 'special':
                ...
                if token.value == '{':
                    ...
                    return {'type': 'close', 'value': token}
                
                elif token.value in ['^', '_']:
                    key = {'_': 'sub', '^': 'sup'}[token.value]
                    ...
                    raise RuntimeError(f"LaTeX parsing error: sorry, special char {token} is not supported yet." + self.parser.error_str)
            ...
            # ----------------------------------------------------------------------------------------------------
                ...
                if cmd in [')', ']']:
                    ...
                    ends = [Token.command('\\right' + c) for c in BRS.values()]
...
if __name__ == "__main__":
    ...
    if False:
        math_mode = True
        tests = maths if math_mode else texts

        for i, s in enumerate(tests):
            lexer = Lexer(s, math_mode=math_mode)
            print("LaTeX:", s)
            pprint(lexer.read_block())
            print()


    lexer = Lexer(r"(x)", math_mode=True)
    print(lexer.read_block())
